#!/bin/bash
# LinuxGSM update_redm.sh module
# Author: Your Name
# Description: Handles updating of RedM servers.

moduleselfname="$(basename "$(readlink -f "${BASH_SOURCE[0]}")")"

artifacts_url="https://runtime.fivem.net/artifacts/redm/build_proot_linux/master/"

fn_update_dl() {
    fn_print_info "Downloading and installing RedM build ${remotebuildversion}"
    tmpfile="${tmpdir}/redm_build_${remotebuildversion}.tar.xz"
...
fn_update_remotebuild() {
    ...
    remotebuildfilename="redm_build_${remotebuildversion}.tar.xz"
